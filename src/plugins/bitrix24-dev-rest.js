(function () {
    ...
    */
    ...
    BX24.initAuth = function (config) {
        ...
        if (PARAMS.AUTH_EXPIRES < new Date().valueOf())
            BX24.refreshAuth(() => {
                util.ready();
                util.init();
            });
    ...
    */
    BX24.callBatch = function (calls, callback, bHaltOnError) {
        var cmd = util.type.isArray(calls) ? [] : {},
            cnt = 0,
            cb = function (cmd) {
    ...
    BX24.callUnBind = BX24.callUnbind = function (event, handler, auth_type, callback) {
        if (!isInit) {
            var _a = arguments;
            BX24.init(function () {
    ...
    BX24.refreshAuth = function (cb) {
        let xhr = new XMLHttpRequest(),
            url =
                "https://" + PARAMS.DOMAIN + "/oauth/token/?grant_type=refresh_token&refresh_token=" + PARAMS.REFRESH_ID + "&client_id=" + PARAMS.CLIENT_ID + "&client_secret=" + PARAMS.CLIENT_SECRET;
        xhr.open("POST", url);
    ...
    BX24.fitWindow = function (cb) {
        sendMessage(
            ...
            {
                width: "100%",
                height: BX24.getScrollSize().scrollHeight,
            },
            cb
        );
    };
    ...
    BX24.proxy = function (func, thisObject) {
        ...
        if (typeof func["__proxy_id_" + proxySalt] == "undefined") func["__proxy_id_" + proxySalt] = proxyId++;
    ...
    BX24.unbind = function (el, evname, func) {
        ...
        if (evname === "mousewheel") BX24.unbind(el, "DOMMouseScroll", func);
    ...
    BX24.openApplication = function (params, cb, settings) {
        ...
        console.log("openApplication launched");
    };
    ...
    BX24.loadScript = function (script, callback) {
        ...
        if (util.type.isString(script)) script = [script];
        var _callback = function () {
            return callback && util.type.isFunction(callback) ? callback() : null;
        };
        var load_js = function (ind) {
            if (ind >= script.length) return _callback();
    ...
    var util = {
        ...
        init: function () {
            var f = function (data) {
                ...
                if (!PARAMS.PLACEMENT_OPTIONS) PARAMS.PLACEMENT_OPTIONS = data.PLACEMENT_OPTIONS;
            ...
            if (util.type.isDomNode(obj)) {
                ...
                if (util.type.isArray(obj)) {
                    ...
                    for (i = 0, l = obj.length; i < l; i++) {
                        if (typeof obj[i] == "object" && bCopyObj) _obj[i] = util.clone(obj[i], bCopyObj);
                        else _obj[i] = obj[i];
                    }
                } else {
                    _obj = {};
        ...
        uniqid: function () {
            var s = "";
            for (var i = 0; i < 32; i++) s += charsList[Math.round(Math.random() * (charsList.length - 1))];
            return s;
        },
    ...
    var sendMessage = function (cmd, params, cb) {
        ...
        if (cmd.indexOf(":") >= 0) {
            ...
            cmd += ":" + (!!params ? JSON.stringify(params) : "") + ":" + sendMessage.setCallback(cb) + (!!PARAMS.APP_SID ? ":" + PARAMS.APP_SID : "");
    ...
    sendMessage.runCallback = function (e) {
        ...
        if (e.origin != "http" + (PARAMS.PROTOCOL ? "s" : "") + "://" + PARAMS.DOMAIN) return;
                        ...
                        if (!!calls[i] && calls.hasOwnProperty(i)) {
                            if (typeof data.result[i] !== "undefined" || typeof data.result_error[i] !== "undefined") {
                                if (util.type.isString(calls[i])) {
                                    ...
                                    q = [util.type.isArray(calls[i]) ? calls[i][0] : calls[i].method, util.type.isArray(calls[i]) ? calls[i][1] : calls[i].data];
                                }
    ...
    ajaxError.prototype.toString = function () {
        return this.ex.error + (!!this.ex.error_description ? ": " + this.ex.error_description : "") + " (" + this.status + ")";
    };
